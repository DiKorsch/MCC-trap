...
class AdaptiveBlobDetector(Blobdetector):
    ...
    def findboxes(self, img, startingid):
        ...
        if self.usebackground: # Substract background image
            h, w = gray.shape
            bg = cv2.resize(self.background, (w, h)).astype(np.float32)
            gray = 255 - np.abs(gray.astype(np.float32) - bg).astype(gray.dtype)
        ...
        # ax1.imshow(gray)
        time2 = time.time()
        logging.debug('Adaptive took {:.3f} ms'.format((time2 - time1) * 1000.0))
